...
const clickEnterOnInput = () => {
  if (dropdownItems.value.length > 0) {
    emit("update:value", dropdownItems.value[0].value);
  } else {
    createAndSelect();
  }
};
...
<template>
  <div v-show="props.isEditing" class="dialog" @click.stop>
    <div style="padding: 6px 8px; background-color: rgba(242, 241, 238, 0.6)">
      <CTag
        v-if="props.value"
        :color="getOptionColor(props.value)"
        :closable="true"
    ...
    <div>
      ...
      <div v-for="item in dropdownItems" :key="item.value" class="options">
        <button @click="emit('update:value', item.value)">
  ...
  <button v-show="!props.isEditing" @click.stop="emit('focus')">
    <CTag v-if="props.value" :color="getOptionColor(props.value)">
      {{ props.value }}
    </CTag>
  </button>
</template>
